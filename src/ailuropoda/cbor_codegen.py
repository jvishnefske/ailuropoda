...
def get_type_info(node, ast):
        ...
        if isinstance(current_node, c_ast.PtrDecl):
            ...
            if current_node.dim:
                try:
                    ...
                    logger.warning(
                        f"Could not determine array size for {current_node.dim.value}. Assuming dynamic or unknown size."
                    )
                    array_size = None
            current_node = current_node.type
        else:
    ...
    if base_type_name == "char" and array_size is not None:
        ...
        "double",
        "bool",
        "_Bool",  # _Bool for direct bool expansion
        "int8_t",
        "int16_t",
        "int32_t",
        "int64_t",
        ...
        "unsigned long long",
        "signed char",
    ]:  # Include signed char as a primitive
        type_category = "primitive"
    elif type_category == "unknown" and base_type_name != "unknown":  # If it's a struct or something else
        if isinstance(current_node, c_ast.Struct) or (
            isinstance(current_node, c_ast.TypeDecl) and isinstance(current_node.type, c_ast.Struct)
        ):
...
def parse_c_string(c_code_string, cpp_path=None, cpp_args=None):
    ...
    try:
        ...
        ast = parse_file(tmp_file_path, use_cpp=True, cpp_path=cpp_path, cpp_args=cpp_args_list)
        return ast
...
def generate_cbor_code(header_file_path, output_dir, templates_dir, cpp_path=None, cpp_args=None):
    ...
    for struct_node in structs_to_generate:
        struct_info = {"name": struct_node.name, "members": []}
    ...
    rendered_header = header_template.render(
        structs=processed_structs, original_header_path=header_file_path.absolute()
    )
...
def main():
    ...
    parser.add_argument(
        "header_file",
        type=Path,
        help="Path to the C header file containing struct definitions.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./generated_cbor"),
