...
def _find_struct(name, file_ast):
    ...
    for node in file_ast.ext:
        if (isinstance(node, c_ast.Decl) and
           isinstance(node.type, c_ast.Struct) and
           node.type.name == name and
           node.type.decls is not None): # Ensure it's a definition, not just a declaration
            return node.type
        elif (isinstance(node, c_ast.Typedef) and
              isinstance(node.type, c_ast.TypeDecl) and
              isinstance(node.type.type, c_ast.Struct) and
              node.type.type.name == name and
              node.type.type.decls is not None):
...
def _expand_in_place(type_node, file_ast, expand_struct=True, expand_typedef=True):
    ...
    if isinstance(type_node, c_ast.TypeDecl):
        type_node.type = _expand_in_place(type_node.type, file_ast, expand_struct, expand_typedef)
        ...
        if type_node.args:
            for i, param in enumerate(type_node.args.params):
                ...
                param.type = _expand_in_place(param.type, file_ast, expand_struct, expand_typedef)
    elif isinstance(type_node, c_ast.Struct):
        # If it's a struct declaration without definition, find its definition
        if type_node.decls is None: # `decls` is None for forward declarations or just `struct S;`
        ...
        if expand_struct and type_node.decls:
            for i, mem_decl in enumerate(type_node.decls):
                ...
                mem_decl.type = _expand_in_place(mem_decl.type, file_ast, expand_struct, expand_typedef)
        ...
        if typedef_def:
            if expand_typedef:
                ...
                return _expand_in_place(copy.deepcopy(typedef_def.type), file_ast, expand_struct, expand_typedef)
...
def _extract_base_type_info(type_node, file_ast):
    ...
    try:
        expanded_type = _expand_in_place(copy.deepcopy(type_node), file_ast, expand_struct=True, expand_typedef=True)
    except RuntimeError as e:
        print(f"Warning: Could not fully expand type for {type_node.__class__.__name__}: {e}", file=sys.stderr)
    ...
    while True:
        if isinstance(current_node, c_ast.TypeDecl):
            ...
            struct_def_node = current_node # This is the expanded struct node with decls
            break
...
def generate_cbor_code_for_struct(struct_node, file_ast):
    ...
    # --- Header Code ---
    header_code.append(f"bool {encode_func_name}(const struct {struct_name}* data, CborEncoder* encoder);")
    header_code.append(f"bool {decode_func_name}(struct {struct_name}* data, CborValue* it);")
    header_code.append("")

    # --- Source Code - Encode Function ---
    source_code.append(f"bool {encode_func_name}(const struct {struct_name}* data, CborEncoder* encoder) {{")
    ...
    source_code.append("")
    source_code.append(f"    err = cbor_encoder_create_map(encoder, &mapEncoder, {len(struct_node.decls)});")
    ...
    for member_decl in struct_node.decls:
        ...
        if not member_name:
            print(f"Warning: Skipping anonymous member in struct '{struct_name}'.", file=sys.stderr)
            continue
        ...
        if member_type_info['is_func_ptr']:
            print(f"Warning: Skipping function pointer member '{member_name}' in struct '{struct_name}' for CBOR codegen.", file=sys.stderr)
            continue

        # Encode key (member name)
        source_code.append(f"    err = cbor_encode_text_stringz(&mapEncoder, \"{member_name}\");")
        source_code.append(f"    if (err != CborNoError) return false;")
        ...
        if member_type_info['is_pointer'] > 0:
            ...
            if member_type_info['is_struct']:
                ...
                source_code.append(f"        if (!encode_{member_type_info['struct_name']}(data->{member_name}, &mapEncoder)) return false;")
            elif 'char' in member_type_info['base_type_names'] and member_type_info['is_pointer'] == 1:
                # char* as null-terminated string
                source_code.append(f"        err = cbor_encode_text_stringz(&mapEncoder, data->{member_name});")
                source_code.append(f"        if (err != CborNoError) return false;")
            else:
                # Pointer to basic type, dereference
                cast_type = _get_c_type_string_for_cast(member_type_info['original_type_node'])
                if 'int' in member_type_info['base_type_names'] or 'char' in member_type_info['base_type_names']:
                    if member_type_info['is_unsigned']:
                        source_code.append(f"        err = cbor_encode_uint(&mapEncoder, (uint64_t)*data->{member_name});")
                    else:
                        source_code.append(f"        err = cbor_encode_int(&mapEncoder, (int64_t)*data->{member_name});")
                elif 'float' in member_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_float(&mapEncoder, *data->{member_name});")
                elif 'double' in member_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_double(&mapEncoder, *data->{member_name});")
                elif 'bool' in member_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_boolean(&mapEncoder, *data->{member_name});")
                else:
                    print(f"Warning: Unsupported pointer type for member '{member_name}' in struct '{struct_name}'. Encoding as null.", file=sys.stderr)
                    source_code.append(f"        err = cbor_encode_null(&mapEncoder);")
                source_code.append(f"        if (err != CborNoError) return false;")
            source_code.append(f"    }}") # End of if (!data->member)
        elif member_type_info['is_array']:
            # Handle arrays
            if 'char' in member_type_info['base_type_names'] and len(member_type_info['array_dims']) == 1:
                # char array as string (e.g., char name[64])
                source_code.append(f"    err = cbor_encode_text_string(&mapEncoder, (const char*)data->{member_name}, strlen((const char*)data->{member_name}));")
                source_code.append(f"    if (err != CborNoError) return false;")
            else:
                # Array of other types or multi-dimensional arrays
                # Encode as CBOR array
                array_len_str = member_type_info['array_dims'][0] # Assuming 1D array for simplicity
                if not array_len_str:
                    print(f"Warning: Skipping indefinite length array member '{member_name}' in struct '{struct_name}'. Requires manual handling.", file=sys.stderr)
                    continue # Skip for now, or encode as empty array
                
                source_code.append(f"    CborEncoder arrayEncoder_{member_name};")
                source_code.append(f"    err = cbor_encoder_create_array(&mapEncoder, &arrayEncoder_{member_name}, {array_len_str});")
                source_code.append(f"    if (err != CborNoError) return false;")
                source_code.append(f"    for (size_t i = 0; i < {array_len_str}; ++i) {{")
                ...
                for _ in range(len(member_type_info['array_dims'])):
                    if isinstance(element_type_node, c_ast.ArrayDecl) or isinstance(element_type_node, c_ast.TypeDecl):
                ...
                cast_type = _get_c_type_string_for_cast(element_type_info['original_type_node'])

                if element_type_info['is_struct']:
                    source_code.append(f"        if (!encode_{element_type_info['struct_name']}(&data->{member_name}[i], &arrayEncoder_{member_name})) return false;")
                elif 'int' in element_type_info['base_type_names'] or 'char' in element_type_info['base_type_names']:
                    if element_type_info['is_unsigned']:
                        source_code.append(f"        err = cbor_encode_uint(&arrayEncoder_{member_name}, (uint64_t)data->{member_name}[i]);")
                    else:
                        source_code.append(f"        err = cbor_encode_int(&arrayEncoder_{member_name}, (int64_t)data->{member_name}[i]);")
                elif 'float' in element_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_float(&arrayEncoder_{member_name}, data->{member_name}[i]);")
                elif 'double' in element_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_double(&arrayEncoder_{member_name}, data->{member_name}[i]);")
                elif 'bool' in element_type_info['base_type_names']:
                    source_code.append(f"        err = cbor_encode_boolean(&arrayEncoder_{member_name}, data->{member_name}[i]);")
                else:
                    print(f"Warning: Unsupported array element type for member '{member_name}' in struct '{struct_name}'. Encoding as null.", file=sys.stderr)
                    source_code.append(f"        err = cbor_encode_null(&arrayEncoder_{member_name});")
                source_code.append(f"        if (err != CborNoError) return false;")
                source_code.append(f"    }}") # End of for loop
                source_code.append(f"    err = cbor_encoder_close_container(&mapEncoder, &arrayEncoder_{member_name});")
                source_code.append(f"    if (err != CborNoError) return false;")
            ...
            if 'int' in member_type_info['base_type_names'] or 'char' in member_type_info['base_type_names']:
                ...
                print(f"Warning: Unsupported basic type for member '{member_name}' in struct '{struct_name}'. Encoding as null.", file=sys.stderr)
                source_code.append(f"    err = cbor_encode_null(&mapEncoder);")
            source_code.append(f"    if (err != CborNoError) return false;")
    ...
    source_code.append(f"bool {decode_func_name}(struct {struct_name}* data, CborValue* it) {{")
    source_code.append("    CborError err;")
    ...
    source_code.append(f"        // If the encoded value is null, set data to default or indicate null.")
    source_code.append(f"        // For simplicity, we'll just advance and return true, assuming caller handles null data.")
    ...
    source_code.append(f"        if (!cbor_value_is_text_string(&mapIt)) return false; // Key must be a text string")
    source_code.append("")
    ...
    for member_decl in struct_node.decls:
        ...
        source_code.append(f"        if (cbor_value_text_string_equals(&mapIt, \"{member_name}\", &num_elements)) {{")
        ...
        if member_type_info['is_pointer'] > 0:
            ...
            if member_type_info['is_struct']:
                # Nested struct pointer
                source_code.append(f"                // WARNING: For struct pointers, memory for data->{member_name} must be allocated by caller.")
                source_code.append(f"                if (!data->{member_name}) {{ /* Handle error or allocate */ return false; }}")
                source_code.append(f"                if (!decode_{member_type_info['struct_name']}(data->{member_name}, &mapIt)) return false;")
            elif 'char' in member_type_info['base_type_names'] and member_type_info['is_pointer'] == 1:
                # char* as null-terminated string
                source_code.append(f"                // WARNING: For char* members, memory for data->{member_name} must be allocated by caller.")
                source_code.append(f"                // This code assumes the buffer is already allocated and large enough.")
                source_code.append(f"                if (!data->{member_name}) {{ /* Handle error or allocate */ return false; }}")
                source_code.append(f"                size_t actual_len_{member_name};")
                source_code.append(f"                err = cbor_value_get_string_length(&mapIt, &actual_len_{member_name});")
                source_code.append(f"                if (err != CborNoError) return false;")
                source_code.append(f"                err = cbor_value_copy_text_string(&mapIt, data->{member_name}, &actual_len_{member_name}, &mapIt);")
                source_code.append(f"                if (err != CborNoError) return false;")
            else:
                # Pointer to basic type, decode into dereferenced location
                source_code.append(f"                // WARNING: For basic type pointers, memory for data->{member_name} must be allocated by caller.")
                source_code.append(f"                if (!data->{member_name}) {{ /* Handle error or allocate */ return false; }}")
                cast_type = _get_c_type_string_for_cast(member_type_info['original_type_node'])
                if 'int' in member_type_info['base_type_names'] or 'char' in member_type_info['base_type_names']:
                    if member_type_info['is_unsigned']:
                        source_code.append(f"                uint64_t temp_val;")
                        source_code.append(f"                err = cbor_value_get_uint64(&mapIt, &temp_val);")
                        source_code.append(f"                if (err != CborNoError) return false;")
                        source_code.append(f"                *data->{member_name} = ({cast_type})temp_val;")
                    else:
                        source_code.append(f"                int64_t temp_val;")
                        source_code.append(f"                err = cbor_value_get_int64(&mapIt, &temp_val);")
                        source_code.append(f"                if (err != CborNoError) return false;")
                        source_code.append(f"                *data->{member_name} = ({cast_type})temp_val;")
                elif 'float' in member_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_float(&mapIt, data->{member_name});")
                elif 'double' in member_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_double(&mapIt, data->{member_name});")
                elif 'bool' in member_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_boolean(&mapIt, data->{member_name});")
                else:
                    print(f"Warning: Unsupported pointer type for member '{member_name}' in struct '{struct_name}'. Skipping decode.", file=sys.stderr)
                    source_code.append(f"                err = cbor_value_advance(&mapIt);") # Advance to skip unknown type
                source_code.append(f"                if (err != CborNoError) return false;")
                source_code.append(f"                // Value was advanced by get_X functions, no need for extra advance here.")
            source_code.append(f"            }}") # End of if (cbor_value_is_null(&mapIt))
        elif member_type_info['is_array']:
            # Handle arrays
            if 'char' in member_type_info['base_type_names'] and len(member_type_info['array_dims']) == 1:
                # char array as string (e.g., char name[64])
                source_code.append(f"            size_t len_{member_name};")
                source_code.append(f"            err = cbor_value_get_string_length(&mapIt, &len_{member_name});")
                source_code.append(f"            if (err != CborNoError) return false;")
                source_code.append(f"            if (len_{member_name} + 1 > sizeof(data->{member_name})) {{ /* Buffer too small */ return false; }}")
                source_code.append(f"            err = cbor_value_copy_text_string(&mapIt, data->{member_name}, &len_{member_name}, &mapIt);")
                source_code.append(f"            if (err != CborNoError) return false;")
            else:
                # Array of other types or multi-dimensional arrays
                array_len_str = member_type_info['array_dims'][0]
                if not array_len_str:
                    print(f"Warning: Skipping indefinite length array member '{member_name}' in struct '{struct_name}' for decoding. Requires manual handling.", file=sys.stderr)
                    source_code.append(f"            err = cbor_value_advance(&mapIt);") # Advance to skip unknown type
                    source_code.append(f"            if (err != CborNoError) return false;")
                    source_code.append(f"            continue;") # Skip to next member
                
                source_code.append(f"            CborValue arrayIt_{member_name};")
                source_code.append(f"            if (!cbor_value_is_array(&mapIt)) return false;")
                source_code.append(f"            err = cbor_value_enter_container(&mapIt, &arrayIt_{member_name});")
                source_code.append(f"            if (err != CborNoError) return false;")
                source_code.append(f"            for (size_t i = 0; i < {array_len_str}; ++i) {{")
                source_code.append(f"                if (cbor_value_at_end(&arrayIt_{member_name})) return false; // Not enough elements")
                ...
                if element_type_info['is_struct']:
                    source_code.append(f"                if (!decode_{element_type_info['struct_name']}(&data->{member_name}[i], &arrayIt_{member_name})) return false;")
                elif 'int' in element_type_info['base_type_names'] or 'char' in element_type_info['base_type_names']:
                    if element_type_info['is_unsigned']:
                        source_code.append(f"                uint64_t temp_val;")
                        source_code.append(f"                err = cbor_value_get_uint64(&arrayIt_{member_name}, &temp_val);")
                        source_code.append(f"                if (err != CborNoError) return false;")
                        source_code.append(f"                data->{member_name}[i] = ({cast_type})temp_val;")
                    else:
                        source_code.append(f"                int64_t temp_val;")
                        source_code.append(f"                err = cbor_value_get_int64(&arrayIt_{member_name}, &temp_val);")
                        source_code.append(f"                if (err != CborNoError) return false;")
                        source_code.append(f"                data->{member_name}[i] = ({cast_type})temp_val;")
                elif 'float' in element_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_float(&arrayIt_{member_name}, &data->{member_name}[i]);")
                elif 'double' in element_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_double(&arrayIt_{member_name}, &data->{member_name}[i]);")
                elif 'bool' in element_type_info['base_type_names']:
                    source_code.append(f"                err = cbor_value_get_boolean(&arrayIt_{member_name}, &data->{member_name}[i]);")
                else:
                    print(f"Warning: Unsupported array element type for member '{member_name}' in struct '{struct_name}'. Skipping decode.", file=sys.stderr)
                    source_code.append(f"                err = cbor_value_advance(&arrayIt_{member_name});") # Advance to skip unknown type
                source_code.append(f"                if (err != CborNoError) return false;")
                source_code.append(f"            }}") # End of for loop
                source_code.append(f"            err = cbor_value_leave_container(&mapIt, &arrayIt_{member_name});")
                source_code.append(f"            if (err != CborNoError) return false;")
        elif member_type_info['is_struct']:
            # Nested struct
            source_code.append(f"            if (!decode_{member_type_info['struct_name']}(data->{member_name}, &mapIt)) return false;")
            ...
            if 'int' in member_type_info['base_type_names'] or 'char' in member_type_info['base_type_names']:
                ...
                print(f"Warning: Unsupported basic type for member '{member_name}' in struct '{struct_name}'. Skipping decode.", file=sys.stderr)
                source_code.append(f"            err = cbor_value_advance(&mapIt);") # Advance to skip unknown type
            source_code.append(f"            if (err != CborNoError) return false;")
        source_code.append(f"        }} else {{")
...
def main():
    ...
    for ext in file_ast.ext:
        ...
        if isinstance(ext, c_ast.Decl) and isinstance(ext.type, c_ast.Struct) and ext.type.decls is not None:
            if ext.type.name: # Only named structs
                struct_definitions[ext.type.name] = ext.type
        # Look for typedef'd structs
        elif isinstance(ext, c_ast.Typedef) and isinstance(ext.type, c_ast.TypeDecl) and isinstance(ext.type.type, c_ast.Struct) and ext.type.type.decls is not None:
            if ext.type.type.name: # Only named structs within typedef
                ...
                dummy_struct_node.name = ext.name # Assign the typedef name as the struct name
    ...
    with open(header_output_path, "w") as f:
        ...
        f.write(f"#include \"{os.path.basename(header_file_path)}\"\n\n") # Include original header
