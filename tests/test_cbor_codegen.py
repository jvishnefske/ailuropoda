...
def test_generate_cbor_code_for_simple_struct():
    ...
    assert (
        "void cbor_encode_Simple(CborEncoder* encoder, const struct Simple* data);"
        == prototype
    )
    assert (
        "int a;" not in generated_code
    )  # These are C declarations, not expected in generated CBOR code
    ...
    assert (
        "cbor_encode_map_start(encoder, &map_encoder, 3);" in generated_code
    )  # 3 members
...
def test_generate_cbor_code_for_struct_with_nested_struct():
    ...
    person_struct_node = _find_struct("Person", file_ast)
    assert person_struct_node is not None  # Ensure struct_node is found
    generated_person_code, person_prototype = generate_cbor_code_for_struct(
        person_struct_node, file_ast
    )

    assert (
        "void cbor_encode_Person(CborEncoder* encoder, const struct Person* data);"
        == person_prototype
    )
    assert "char name[64];" not in generated_person_code
    assert "struct Point location;" not in generated_person_code
    assert (
        'cbor_encode_text_string(&map_encoder, "name", strlen("name"));'
        in generated_person_code
    )
    assert (
        "cbor_encode_text_string(&map_encoder, data->name, strlen(data->name));"
        in generated_person_code
    )
    assert (
        'cbor_encode_text_string(&map_encoder, "location", strlen("location"));'
        in generated_person_code
    )
    assert (
        "cbor_encode_Point(&map_encoder, &data->location);" in generated_person_code
    )  # Expect call to nested encoder
    assert (
        "cbor_encode_map_start(encoder, &map_encoder, 2);" in generated_person_code
    )  # 2 members
    ...
    generated_point_code, point_prototype = generate_cbor_code_for_struct(
        point_struct_node, file_ast
    )
    assert (
        "void cbor_encode_Point(CborEncoder* encoder, const struct Point* data);"
        == point_prototype
    )
    assert (
        'cbor_encode_text_string(&map_encoder, "x", strlen("x"));'
        in generated_point_code
    )
    assert "cbor_encode_int(&map_encoder, data->x);" in generated_point_code
    assert (
        'cbor_encode_text_string(&map_encoder, "y", strlen("y"));'
        in generated_point_code
    )
...
def test_generate_cbor_code_for_empty_struct():
    ...
    assert (
        "void cbor_encode_EmptyStruct(CborEncoder* encoder, const struct EmptyStruct* data);"
        == prototype
    )
